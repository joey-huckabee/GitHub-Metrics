...
EXPECTED_HEADER = (
    "owner,organization,scan_date,scan_id,stars,forks,age_days,"
...
def test_the_reference_row_renders_exactly_as_documented(reference_row: SoftwareRow) -> None:
    ...
    assert rendered == (
        "cline,cline,2026-07-12 20:33:07.254804+00:00,"
...
def test_an_unfetchable_row_keeps_its_identity_and_empties_the_rest(
    ...
    # none. Only the columns that come from the input row and the scan survive.
    assert cells[:2] == ["ghost", ""]
    assert cells[2] == str(SCAN_DATE)
    assert cells[3] == str(SCAN_ID)
